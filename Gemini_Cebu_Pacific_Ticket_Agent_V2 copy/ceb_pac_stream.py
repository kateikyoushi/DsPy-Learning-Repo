...
║  Powered by DSPy-Optimized AI (64% Quality)                             ║
║                                                                          ║
...
Features:
- 💬 Real-time chatbot with optimized agent (64% quality)
...
To run:
$ streamlit run cebu_pacific_gemini_chatbot.py
"""
...
class Config:
    ...
    AGENT_FILE = "cebu_pacific_optimized_agent_flash.json"
    RESULTS_FILE = "optimization_results.json"
    MLFLOW_URI = st.secrets.get("MLFLOW_TRACKING_URI", os.getenv("MLFLOW_TRACKING_URI", "http://localhost:8080"))
...
def load_optimized_agent() -> Optional[SupportAgent]:
    ...
    try:
        ...
        if not Config.GEMINI_API_KEY:
            st.error("⚠️ GEMINI_API_KEY not configured! Please set it in Streamlit secrets.")
            st.code("In Streamlit Cloud dashboard: Secrets -> Add GEMINI_API_KEY")
            return None

        # Configure DSPy with Google Gemini
        lm = dspy.LM('gemini/gemini-3-flash-preview', api_key=Config.GEMINI_API_KEY)
        ...
        if os.path.exists(Config.RESULTS_FILE):
            ...
            return {
                "baseline_performance": {
                    "avg_quality_score": 0.4882,
                    "avg_response_time_seconds": 6.16,
                    "min_score": 0.205,
                    "max_score": 0.84,
                    "std_dev": 0.158
                },
                "optimized_performance": {
                    "avg_quality_score": 0.6391,
                    "avg_response_time_seconds": 5.37,
                    "min_score": 0.321,
                    "max_score": 0.92,
                    "std_dev": 0.167
                },
                "improvements": {
                    "quality_score_gain": 0.1509,
                    "quality_score_gain_pct": 30.91,
                    "response_time_reduction_pct": 12.71
                },
                "business_impact": {
...
def render_chat_tab():
    ...
    with st.sidebar:
        ...
        agent = load_optimized_agent()
        if agent:
            st.success("✅ Agent Loaded (64% Quality)")
            st.info("🚀 Model: gemini-3-flash-preview")
            st.info(f"🔌 API: {'Connected' if Config.GEMINI_API_KEY else 'Not configured'}")
        else:
...
def render_about_tab():
    ...
    optimized using **DSPy's MIPROv2** algorithm, achieving a **31% improvement** 
    in response quality (from 49% to 64%).

    ### What makes this special?

    - ✅ **Automated optimization**: No manual prompt engineering needed
    - ✅ **Data-driven**: Learned from 626 real support interactions
    - ✅ **Measurable results**: 64% quality score vs 49% baseline
    - ✅ **Fast responses**: ~5.4 seconds average
    ...
    with col1:
        ...
        **LLM & Inference:**
        - 🤖 **Google Gemini**: Advanced multimodal LLM
        - ⚡ **Gemini 3 Flash Preview**: Latest fast model
        - 🚀 **Optimized DSPy**: Declarative self-improving prompts
        """)
    ...
    with col2:
        ...
        **Data & Evaluation:**
        - 📝 **626 training examples**: Real support tickets from 176 articles
        - ✅ **157 validation examples**: Test scenarios
    ...
    with st.expander("**Q: How accurate are the responses?**"):
        st.markdown("""
        **A:** The agent achieves 64% quality score based on our evaluation metric,
        which checks for:
    ...
    with st.expander("**Q: What's the cost to run this?**"):
        ...
        **A:** The costs are minimal:
        ...
        - **Inference**: ~$0.0001 per query with Gemini Flash
    ...
    **License:** MIT
    ...
    <div style='text-align: center; color: #666; padding: 20px;'>
        <p>Made with ❤️ using DSPy + Google Gemini + Streamlit</p>
...
def render_knowledge_tab():
    """Render the knowledge base tab with scraped information"""

    st.title("📄 Knowledge Base")
    st.markdown("Review the scraped Cebu Pacific help center information used for training the agent")

    # Load the help center content
    try:
        with open("cebu_pacific_helpcenter.md", "r", encoding="utf-8") as f:
            content = f.read()
        
        st.markdown("### Cebu Pacific Help Center Content")
        st.markdown(content)
        ...
        st.error("❌ cebu_pacific_helpcenter.md not found in the current directory")
        st.info("Please ensure the scraped help center file is present.")
    except Exception as e:
...
def main():
    ...
    <div style='text-align: center; padding: 20px 0;'>
        ...
        <p style='font-size: 1.2em; color: #666;'>
            Powered by DSPy-Optimized AI · 64% Quality Score · 31% Improvement
        </p>
    </div>
    """, unsafe_allow_html=True)
